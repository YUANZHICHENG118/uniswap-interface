...
import { useTokenContract, useLpContract } from '../../hooks/useContract'
...
const PageWrapper = styled(AutoColumn)`
  ...
  width: 100%;
`
export default function EarnToken({
                               match: {
                                 params: { symbol, type }
                               }
  ...
  const min=1;
  // 代币合约
  const token_Contract = useTokenContract(UNI[chainId??1].address, true)
  // 代币合约
  const tokenStakeContract = useLpContract(LPCONTRACT, true)

  // 我的余额
  const balanceOfResult = useSingleCallResult(token_Contract, 'balanceOf', [account??undefined])
  ...
  // 授权数量
  const allowanceResult = useSingleCallResult(token_Contract, 'allowance', [account ?? undefined, LPCONTRACT])
  const approveAmount=allowanceResult?.result?.[0]??0;
  const approveToken=new TokenAmount(UNI[chainId??1],approveAmount)
  ...
  const userStakeAmountToken=new TokenAmount(UNI[chainId??1],userStakeAmount)
  // const staketime=userStakeResult?.result?.["staketime"];
  ...
  const approve=async ()=>{
    if(token_Contract) {
      ...
      const estimatedGas = await token_Contract.estimateGas.approve(LPCONTRACT, _amount).catch(() => {
        // general fallback for tokens who restrict approval amounts
        return token_Contract.estimateGas.approve(LPCONTRACT, _amount)
      })

      return token_Contract.approve(LPCONTRACT, _amount, {
  ...
  const onFinish = async (values: any) => {
    ...
    if(token.lessThan(JSBI.BigInt(values['stakeAmount']))){
      notification.error({
        message: t("tip"),
        description:
          t("balanceLow"),
  ...
  const reap=async ()=>{
    ...
    if(tokenStakeContract){
      ...
      const estimatedGas = await tokenStakeContract.estimateGas.reap(data?.pid).catch(() => {
      ...
      <PageWrapper>
        ...
        <Card
          ...
          actions={[
            ...
            <Button loading={btnLoading} disabled={parseInt(userStakeAmountToken.toSignificant())<=0} type="primary" style={{ width: '90%' }} onClick={unStake}>
              Withdraw
            </Button>
          ]}
        >
        ...
        <Modal title="Stake" visible={showModel} onOk={onFinish} footer={null} onCancel={()=>{setShowModel(false)}}>
          ...
          >
            <Form.Item>
              <RowBetween>
                {/*总量*/}
                <span>{t("balance")}{token.toFixed(4)} PIPE</span>
                {/*已完成*/}
              </RowBetween>
            </Form.Item>
            ...
            >
              <Input  type={'number'} size="large" addonAfter="PIPE " placeholder={t('p-input')+'PIPE'}
                     />
            </Form.Item>
