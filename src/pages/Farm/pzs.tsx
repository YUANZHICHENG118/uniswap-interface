...
import { supportedPools, pzsToken, PZS_ADDRESS } from '../../constants/index'
...
`
export default function FarmTokenPzs(props: RouteComponentProps<{ symbol: string }>) {
  const {
    match: {
      params: { symbol }
    }
  ...
  const contract = usePzsContract(PZS_ADDRESS, true)
  ...
  const approvalHandel= async ()=>{
    if(lpcontract){
      ...
      const estimatedGas = await lpcontract.estimateGas.approve(PZS_ADDRESS,_amount).catch(() => {
        ...
        return lpcontract.estimateGas.approve(PZS_ADDRESS,_amount)
      })

      return lpcontract.approve(PZS_ADDRESS,_amount, {
