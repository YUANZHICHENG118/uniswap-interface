...
import { supportedPools, pzsToken, PZSS_ADDRESS } from '../../constants/index'
import { RouteComponentProps } from 'react-router-dom'
import { usePzsContract ,useTokenContract} from '../../hooks/useContract'
...
`
export default function FarmTokenPzss(props: RouteComponentProps<{ symbol: string }>) {
  const {
    match: {
      params: { symbol }
    }
  ...
  const token = supportedPools[4]

  const lpcontract = useTokenContract(token && token.tokenAddresses, true)

  const contract = usePzsContract(PZSS_ADDRESS, true)


  const allowance = useSingleCallResult(lpcontract, 'allowance', [account ?? undefined, PZSS_ADDRESS])
  ...
  const approvalHandel= async ()=>{
    if(lpcontract){
      ...
      const estimatedGas = await lpcontract.estimateGas.approve(PZSS_ADDRESS,_amount).catch(() => {
        // general fallback for tokens who restrict approval amounts
        return lpcontract.estimateGas.approve(PZSS_ADDRESS,_amount)
      })

      return lpcontract.approve(PZSS_ADDRESS,_amount, {
  ...
  return (
    <MenuWrap className='container'>
      ...
      <Modal isOpen={visibleModal} onDismiss={()=>setVisibleModal(false)} minHeight={20} maxHeight={390} >
        <WalletBox>
          <h2>
            {format(lpBalance&&lpBalance.toString(),token&&token.decimals||18,8)} {token && token.symbol} Avaliable
          </h2>
