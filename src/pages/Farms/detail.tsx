  ...
  const findBalance = () => {
    if (tokens) {
      if (tokens.symbol === 'TRX') {
        ...
        balanceOf(tokens.lp?tokens.poolAddress:tokens.address, tokens.lp).then((data: any) => {
  ...
  return (
    <MenuWrap>
      ...
      <div
        ...
        onClick={() => (stakeBalance > 0 ? handelExit() : console.error('not balance'))}
        style={{ color:  stakeBalance > 0 ? '' : 'rgba(209, 0, 75, 0.333)' }}
      >
        {t('HarUnst')}
      </div>
